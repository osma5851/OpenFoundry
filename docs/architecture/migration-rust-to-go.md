...
| 6 | ML/AI/apps & retire Rust | 🟡 in progress — all 8 architecture+slice / foundation / substrate ports done: `code-repository-review-service` ✅, `media-transform-runtime-service` ✅, `entity-resolution-service` ✅, `ontology-exploratory-analysis-service` ✅, `reindex-coordinator-service` ✅, `lineage-service` ✅, `workflow-automation-service` ✅, `federation-product-exchange-service` ✅ (substrate + 8 migrations + ListResponse[T] envelope + SyncStatus + NexusOverview shared model scaffolding for the 3 absorbed sub-domains: marketplace, marketplace-catalog, product-distribution). Inventory at openfoundry-go/INVENTORY-PHASE6.md. Remaining: libs/ai-kernel-go skeleton + 8 ai-kernel-bound shells (multi-iteration); pyo3 sidecars STOP-and-ask. Per-service follow-up slices queued in TodoList. |
